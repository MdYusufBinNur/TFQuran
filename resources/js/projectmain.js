/**
 * MdYusufBiNur
 * This Js File is for this project only. It contain all functionality.
 * There is no way to use this in another project
 */
    $(function () {
        $("#example1").DataTable();
        $('#example2').DataTable({
    ...
    };jQuery( function($) {$('#message').fadeOut (550);$('#message').fadeIn (550);$('#message').fadeOut (550);$('#message').fadeIn (550);$('#message').fadeOut (550);$('#message').fadeIn (550);$('#message').fadeOut (550);});$(document).on('click', '.ts-delete-alt', function (e) {e.preventDefault();var id = $(this).data('id');
        // var userId = $(this).data('user_id');
        // console.log(userId);
        swal({title: "Are you sure want to delete?"+id, type: "error", icon: "warning", buttons: [true, "Delete"], dangerMode: true,})});/*    $(document).on('click', '.ts-delete-al', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
        ...
        });*/function delete_ayah(id) {$("#del_ayah").attr("href", "/del_ayah/" + id);}function updatedSura(sura_no,ayah_no,token_no) {
        ...
        //alert(ayah_no);
    }function delete_editor(id) {$("#del").attr("href", "/delete_editor/" + id);}function set_role(id) {$('#rol_id').val(id)}function updatedEditors(id) {$.ajax({
            url: 'editor_info/'+id,
            method: 'get',
            success: function (data) {
        ...
        })}function delete_author(id) {$("#del").attr("href", "/delete_author/" + id);}function updatedAuthor(id) {
        //alert(id)
        $.ajax({
            url: 'author_info/'+id,
            method: 'get',
    ...
    });$('.update_record_form').hide();$('#update_ayah_record').find('input[type="checkbox"]').click(function(){
        if($(this).prop("checked") == true)
        {
    ...
    }let rep = $('#sura_rmve_col').html();rep.replace(/['"]+/g, '');function getAyahs(id) {
